...
// Memoized components
const UserMessageBubble = React.memo(({ message }: { message: ChatMessage }) => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-end mb-4">
        <div className="flex items-start gap-3 max-w-[80%]">
            <div className="bg-white text-black rounded-2xl rounded-br-md px-4 py-3 shadow-lg">
                ...
                {message.attachments && message.attachments.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                        {message.attachments.map((file, index) => (
                            <div key={`${file.name}-${index}`} className="flex items-center gap-2 bg-gray-100 rounded-lg px-2 py-1">
                ...
                <div className="text-xs text-black/60 mt-2 text-right">
                    {message.timestamp}
                </div>
            </div>
            <div className="flex-shrink-0 w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-md">
...
const AIMessageBubble = React.memo(({ message, onCopy, onRegenerate, onReaction }: { message: ChatMessage; onCopy: (content: string) => void; onRegenerate: (id: string) => void; onReaction: (id: string, reaction: 'up' | 'down') => void; }) => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3 mb-6 group">
        ...
        <div className="flex-1 min-w-0">
            <div className="text-zinc-100 leading-relaxed text-sm whitespace-pre-wrap min-h-[20px]">
                {message.isTyping && !message.content ? (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-zinc-400 text-xs">Shadow is thinking...</span>
                    </div>
                ) : (
                    message.content
                )}
            ...
            {message.content && !message.isTyping && (
                <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="sm" onClick={() => onCopy(message.content)} className="h-6 px-2 text-zinc-400 hover:text-white cursor-pointer">
                            <Copy className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => onRegenerate(message.id)} className="h-6 px-2 text-zinc-400 hover:text-white cursor-pointer">
                            <RefreshCw className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => onReaction(message.id, 'up')} className="h-6 px-2 text-zinc-400 hover:text-green-400 cursor-pointer">
                            <ThumbsUp className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => onReaction(message.id, 'down')} className="h-6 px-2 text-zinc-400 hover:text-red-400 cursor-pointer">
...
export default function ShadowChatModal({
    ...
    const processMessage = useCallback(async (message: string, attachments: File[] = []) => {
        ...
        try {
            if (attachments.length > 0) {
                toast.warning("File attachments are not yet fully supported in chat.");
            }
            ...
            if (result.success && result.data?.response) {
                setMessages(prev => prev.map(msg => 
                    msg.id === assistantMessageId 
                        ? { ...msg, content: result.data.response, isTyping: false } 
                        : msg
                ));
            } else {
    ...
    }, []); // This useCallback has no external dependencies from the component scope
    ...
    }, [isOpen, initialMessage, initialAttachments, messages.length, generateContextualWelcome, processMessage, recentContext, userMission]);
