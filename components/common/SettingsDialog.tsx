...
import {
  Dialog,
  DialogContent,
  DialogTitle, // --- THIS IMPORT IS NOW NEEDED AGAIN ---
...
export default function SettingsDialog({ user, profile, trashViewContent, open, onOpenChange }: SettingsDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl w-full h-[640px] p-0 gap-0 border-0">
        <Tabs defaultValue="general" className="grid grid-cols-4 h-full">
          
          <div className="col-span-1 flex flex-col items-start gap-4 p-4 border-r border-border/80">
            {/* --- THIS IS THE FIX: Use <DialogTitle> instead of <h2> --- */}
